...
function renderEp1File($ep1Filename)
{
    ...
    // read EP1 file
    global $level1Data, $pageBuffer, $level15, $x26Present;
    ...
    for ($row = 0; $row <= 23; $row++) {
        ...
        for ($col = 0; $col <= 39; $col++) {
            ...
            if ($attributesChanged || $doubleWidthMode) {
                ...
                $newFgAttributes =
                 ...
                 $ttxAttributes['flash'] . ' ' . $ttxAttributes['size'];
        ...
        if ($subpageNum < $numSubpages)  {
            $htmlBuffer = preg_replace('/(-&gt;|&gt;&gt;)/', '<a href="?page=' . $pageNum . '&sub=' . $nextSubpageNum . '">${1}</a>', $htmlBuffer);
        }
        else {
            $htmlBuffer = preg_replace('/(-&gt;|&gt;&gt;)/', '<a href="?page=' . $nextPageNum . '">${1}</a>', $htmlBuffer);
        }        
    ...
    echo '<!-- generated by ttxweb EP1 teletext document renderer version: ' . TTXWEB_VERSION . ' (c) 2023 Fabian Schneider -->' . "\n";
...
function decodeX26Chars($x26Data)
{
    ...
    $g2SupplementaryChars = array(0x50 => '&#8212;', 0x52 => '&#174;', 0x53 => '&#169;', 0x54 => '&#169;', 0x55 => '&#9834;', 0x56 => '&#8364;');

    // the G2 diacritical marks according to ETSI EN 300 706 clause 15.6.3
    $g2DiacriticalMarks = array(1 => 'grave', 2 => 'acute', 3 => 'circ', 4 => 'tilde', 10 => 'ring');
